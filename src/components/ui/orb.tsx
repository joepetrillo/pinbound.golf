...
// @react-three/fiber still constructs THREE.Clock (deprecated since r183)
// with no way to inject a replacement, so filter that one deprecation notice
// while letting every other three.js log through untouched.
THREE.setConsoleFunction((type, message, ...params) => {
  if (message.startsWith("THREE.Clock: This module has been deprecated")) {
    return;
  }
...
export function Orb({
  ...
  useEffect(() => {
    ...
    const mountWhenReady = () => {
      ...
      raf = requestAnimationFrame(mountWhenReady);
    };
  ...
  return (
    <div className={className ?? "relative h-full w-full"} ref={containerRef}>
      {canvasKey === null ? null : (
        <Canvas
          key={canvasKey}
