...
export const LiveWaveform = ({
  ...
  // Keep callbacks out of the mic effect deps — unstable identities would
  // restart getUserMedia and can leak tracks (see cancelled flag below).
  const onErrorEvent = useEffectEvent((error: Error) => {
    onError?.(error);
  });
  const onStreamReadyEvent = useEffectEvent((stream: MediaStream) => {
    onStreamReady?.(stream);
  });
  const onStreamEndEvent = useEffectEvent(() => {
    onStreamEnd?.();
  });
  ...
  // getUserMedia is async — without a cancelled flag, a stream that resolves
  // after cleanup (Strict Mode remount, dep change, or fast stop) is never
  // stopped and the browser keeps showing the mic as in use.
  useEffect(() => {
    ...
    const setupMicrophone = async () => {
      try {
        ...
        // Effect already cleaned up — stop this stream immediately so it
        // cannot leak past the browser's mic indicator.
        if (cancelled) {
